...
def _qbx_rows(nystrom: dict) -> dict[str, dict]:
    ...
    target_t = np.mod(
        np.arctan2(
            (points[:, 1] - CENTER[1]) / SEMI_MINOR,
            (points[:, 0] - CENTER[0]) / SEMI_MAJOR,
        ),
        2.0 * np.pi,
    )
    strategies = {
        ...
        "qbx_fourier8": (
            gpr_bem_qbx.FullRowQBX(
                ...
                source=gpr_bem_qbx.ParameterizedFourierSources(
                    parameterization=ellipse_parameterization(CENTER, SEMI_MAJOR, SEMI_MINOR),
