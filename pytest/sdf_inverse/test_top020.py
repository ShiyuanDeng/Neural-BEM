...
from experiments.top020 import summarize as summary
...
def test_saved_completed_fresh_run_replays_frequency_counter_fields():
    bundle=r.ROOT/'results/validation/topology/TOP-020-20260915-153359-fresh-two-stars'
    if not (bundle/'artifact_manifest.json').exists():
        pytest.skip('completed TOP-020 bundle not available in this checkout')
    verification,endpoints=summary.verify(bundle)
    assert verification['status']=='PASS' and verification['endpoints_verified']==5
    assert not verification['fresh_recovery_pass'] and verification['total_attempted_frequency_calls']==9375
    assert endpoints[-1][1]['score']['geometry']['component_count']==2
