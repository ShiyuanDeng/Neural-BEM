...
> [`current_architecture.md`](architecture_snapshot_2026-09-06.md); for active work see
> [`ordered_boundary_nystrom_plan.md`](../legacy/ordered_boundary_nystrom_plan.md).
> The compressed-cloud QBX/kdiff investigation closed on 2026-09-01; its
> evidence and qualifications are in [`qbx_closure.md`](../legacy/qbx_closure.md).
...
decision in [`qbx_closure.md`](../legacy/qbx_closure.md); do not continue them as the
...
see [`qbx_closure.md`](../legacy/qbx_closure.md).
...
The decision in [`qbx_closure.md`](../legacy/qbx_closure.md) stops further tuning on the
...
[`docs/qbx_closure.md`](../legacy/qbx_closure.md).
...
[`results/sdf_boundary_parameterization/kress-scalar-proxy-20260902/summary.md`](../../results/validation/sdf_boundary_parameterization/kress-scalar-proxy-20260902/summary.md).
...
[`gpr_bem_kress_implementation.md`](../reference/gpr_bem_kress_implementation.md).
...
are frozen in
[`results/solver_comparisons/kress-peer-20260902/summary.md`](../../results/validation/solver_comparisons/kress-peer-20260902/summary.md).
...
[`results/inverse_solver_comparison/README.md`](inverse_results_through_2026-09-06.md).
...
Read the complete [priority brief](../legacy/codex_sdf_kress_priorities_2026-09-05.md)
...
## 2026-09-07 — Results and documentation organization

The user clarified that strict MLP-owned geometry with Method B is the
pipeline to repair, not merely a retained comparison. Radial, representation,
material and metric findings now explicitly inform that objective in the
[living pipeline documents](../README.md) and [results catalogue](../../results/README.md).
This organization does not implement the numerical repair.

The cleanup relocated whole run bundles by actual pipeline and evidence role,
archived superseded MLP-feedback/normal-update/development implementations,
and separated demonstrations from inverse recovery. Seven root logs now sit
beside their matching bundles as `stdout.log`. Dated reports, technical
references, and superseded plans have distinct documentation locations.
The catalogue has 153 rows across 51 bundles, including two formerly empty
folders labelled as no saved experiment. Dates come from recorded metadata
where available; missing dates and directory-only labels remain explicit.

All 950 original non-Markdown data/log payloads were verified byte-identical,
all original files remain present, and the signed scalar-proxy outputs and
frozen-curve hashes still validate. One documented geometry compatibility
symlink preserves recorded paths and the user's existing notebook, whose
pre-cleanup bytes are unchanged. Live driver edits only update artifact
inputs and default output paths; solver, optimizer and numerical settings
were not changed. Previously tracked binary snapshots remain eligible for
Git at their new paths; other local ignored artifacts were moved with their
bundles.

Verification was static: file hashes and existence, catalogue identities and
metadata paths, local Markdown links, Python syntax without importing drivers,
Bash syntax for the provided command blocks, and whitespace checks. **No
pytest suite, solver, fitting run, or inverse experiment was executed.**
The [organization audit](../../results/organization_audit.json) records the
checks; [reproduction](../reproduction.md) provides commands for the user's
later runs and distinguishes runnable controls from pending strict-pipeline
repair experiments. Original measured commands and source hashes remain
historical provenance rather than being rewritten as new measurements.
