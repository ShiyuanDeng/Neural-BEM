# Single-object shape and material inverse controls
...
radial geometry. They supply derivative, objective and starting-basin
controls useful for the [strict MLP + Method-B repair](strict_mlp_method_b.md).
They do not implement that MLP-owned path.
...
supports this fixed-topology, coherent-curve contract. Coupling those
derivatives to actual MLP/Method-B updates requires the additional transfer
checks described in the strict pipeline page.

## Drivers and saved evidence

| Driver / run | Scene and recorded date | Measured takeaway |
|---|---|---|
| [Material comparison](../../run_material_inverse_comparison.py); [eight-arm result](../../results/inverse/shape_material/material_inverse/bounded-20260906/README.md) | Mie circle at `(0.5,0.5)` m, radius `0.05` m, interior `epsr=3`; fixed/joint fits, clean and 1% complex noise; 2026-09-06 | Five of eight arms pass physical recovery. Three high-permittivity starts fail; local optimizer termination does not certify recovery |
| [Robustness comparison](../../run_material_robustness_comparison.py); [fifteen-workflow result](../../results/inverse/shape_material/material_robustness/bounded-20260906/README.md) | Historical circle plus independent Nyström noncircle with interior `epsr=8.4`; clean/noisy joint cohorts and fixed-circle control; 2026-09-06 | Full-band local policy passes 0/5 cohorts, continuation 2/5, bounded multistart continuation 5/5 |
...
## What should carry into the strict inverse repair
...
data, not optimization inputs. An MLP-based implementation needs equivalent
contracts for its actual extracted geometry before these controls can be
interpreted as a fair comparison.
...
automatically the tolerances for a future strict MLP experiment.
