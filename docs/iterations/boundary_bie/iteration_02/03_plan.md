...
- Execution: **COMPLETE**, 2026-09-15; [measured closeout](../../../../results/validation/boundary_bie/BIE-004-20260915-coupled-02/README.md).
...
All scientific stages and all three timing pairs completed. The coupled
analytic derivative passes the declared fixed-topology accuracy and cost gates:
full 34-direction Jacobian errors are 1.414e-7 (COMMON) and 1.405e-7 (terminal),
with worst column error 4.647e-7. All 1,008 scaled operator checks pass. Worst
256/512 analytic sensitivity discrepancy is 9.500e-12. Taylor ratios are
4.00003–4.00056. The repeated terminal-state full Jacobian costs 26.249 s
analytic versus 49.083 s FD, a 1.870x median speedup, with nonoverlapping ranges.
No other repository numerical worker was detected at timing-arm boundaries.

Work: 703/800 assembly equivalents, 200/300 analytic directions, 504/600
factorizations, 703/1000 RHS batches, 595.58/1800 numerical seconds and
1.690/8 GiB peak RSS. The first attempt stopped on one malformed negative test
before any physical work; the permitted correction fixed only that test input.
Its elapsed time and failed evidence are retained in the continuation.

The original plan is preserved byte-for-byte in the bundle's `approved_plan.md`.
All source/input hashes and saved Jacobian error replay pass. Results open
[iteration 03](../iteration_03/01_results.md). The next recommendation is one
opt-in integration and matched inverse validation, coordinating shared edits
with topology's measurement windows. No shared production code, inverse,
optimizer default, branch, worktree, commit or push was changed by this task.
