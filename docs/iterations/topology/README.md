...
| Execution status | TOP-008 `COMPLETE`: the feasible-FD stencil frees the pinned component (8.000 mm to 22.737 mm), returns 10/12 runs against 9/12, improves `split` to 0.000023 mm at a quarter of the solves — but buys **no new benchmark pass**, still 5/12. TOP-009 `STOPPED AT STAGE 2, READING CORRECTED`: bandwidth enrichment does produce real shape, and the optimizer parks it 34 degrees out of rotational phase while the ladder stops on its solve cap three rungs short. 578 tests pass; all three new mechanisms ship opt-in and off; no controller default changed |
| Next expected action | **Globalization, not regularization and not acquisition — and not authorized.** Stage 3 refuted the data hypothesis: the true geometry fits the training acquisition to 3.23e-07 relative error, so the climb's answer is 7.1e6x worse than achievable. What blocks recovery is a local minimum in rotational phase plus a truncated ladder. Candidates in evidence order: (1) raise the ladder's solve cap so a component can reach the bandwidth its truth needs — cheapest, and until it is done no full-ladder result means anything; (2) seed promoted modes from a contour fit instead of zeroing them, since zero is the least determined phase; (3) try several phases at promotion and keep the best by training objective. Separately unresolved: whether `refined_feasibility_guard`, `feasible_fd_jacobian` and `bandwidth_promotion` become defaults — one declared comparison, not three closeouts |
...
| Blockers | Three explanations are now tested and eliminated in order: the derivative (fixed, insufficient), capacity (restored, insufficient) and data/regularization (refuted — the truth fits to 3.23e-07). What remains is an optimization problem. Three scenes still time out and noncircular recovery is unresolved |
