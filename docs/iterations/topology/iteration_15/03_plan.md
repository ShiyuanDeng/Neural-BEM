...
396 new calls, all completed, zero failed, 274.108536 active seconds. Full model
replay and the two selected directional checks pass. All four candidate arms
complete; 1e-2 alone passes the >=2× gain/no-more-calls gate (3.978× gain, 8
versus 24 candidate calls). Coordinate clipping is inactive in all proposals.
All 202 sources/inputs remain unchanged. 85 pre-dispatch tests and saved-array
replay pass. No inverse was run and no recovery/full-suite claim is made.
Results open [iteration 16](../iteration_16/01_results.md), whose next bounded
comparison must test whether the initial damping reset improves reconstruction.
