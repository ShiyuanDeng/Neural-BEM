...
Once the user approves an ID and its conditional stages, agents may proceed through stages whose gates pass without repeatedly asking for the same approval. They may not change the scope or authorize another ID. Concurrent numerical work uses separate branches/worktrees and must not mutate a checkout being measured.
