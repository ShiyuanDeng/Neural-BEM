...
| Item | State |
|---|---|
| Active cycle | [Iteration 22 — SC-040: the current pipeline on all six scenes, with atlas videos](iteration_22/01_results.md), after [iteration 21 — SC-039 trajectory data](iteration_21/01_results.md) |
| Stage | **SC-040 COMPLETE, 2026-09-25.** State band then M 11/15/19 release on all six scenes (C, kite from SC-038). Final RMS: circle 0.00024, star 0.142, C 0.0241, kite 0.110, peanut 0.0104, hook 0.0297 mm; all new endpoint audits pass. Six atlas videos, one per scene, replace the three-method videos in `videos/` (kept in `videos/three_method_comparison/`). The atlas puts star's remaining misfit at m=20–22, just past M=19. **SC-039 (previous):** raw boundary data for 867 states of 34 trajectories |
