...
| Stage | **SC-034 APPROVED by user direction; IN PROGRESS.** SC-030's SPD arm failed circle and star, which the legacy single-object inverse recovers. The [assessment](iteration_15/02_proposals/01_spd_legacy_controls.md) restores five legacy controls to the SPD comparison fitter as opt-in `StepSafeguards` plus a K 4/6/8/10 ladder; the [SC-034 plan](iteration_15/03_plan.md) tests them on the six cases. SC-032 result unchanged: R0 stays the hybrid baseline, R2 opt-in |
...
| Review / next decision | Finish SC-034 and apply its pre-declared adoption rule for the SPD comparison reference. SC-033 (Borges eq. 13 admissibility filter) remains proposed, not approved, and waits for the corrected baseline |
