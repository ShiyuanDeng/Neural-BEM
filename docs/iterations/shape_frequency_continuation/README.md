...
| Review / next decision | SPD-L adopted as the comparison reference. **Next, the user's decision on** a re-scoped successor. [RD-2](iteration_16/02_proposals/01_band_energy_audit.md) and [RD-3](iteration_16/02_proposals/02_step_span_audit.md) show the update band does not set shape complexity, and that the hybrid's collapse is a finite-step validity limit of normal moves near sub-resolution features. [RD-4](iteration_16/02_proposals/03_storage_band_audit.md) proposes the concrete form: **SC-035, a per-stage storage-band ladder** in the hybrid (Bernstein: arclength band K bounds the radius below by L/(2πK)). It supersedes SC-033 as written. Not approved. SC-033 should be re-scoped against SC-035. Both are listed in [iteration 16](iteration_16/01_results.md); neither is approved |
