...
| Review / next decision | **The user's decisions:** (1) adopt **SPD-L** (ladder only) instead of LS as the SPD comparison reference (recommended); (2) the proposed **SC-035**, a band-limited hybrid state, which tests the mechanism SC-034 exposes. SC-033 should be re-scoped against SC-035. Both are listed in [iteration 16](iteration_16/01_results.md); neither is approved |
