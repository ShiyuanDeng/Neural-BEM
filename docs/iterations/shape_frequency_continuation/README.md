...
| Stage | **SC-030 COMPLETE; proposal review stage.** The user-supplied [research brief](iteration_13/02_proposals/01_atlas_to_inverse_research_brief.md) (mechanism → prediction → intervention → full-inverse benefit) is placed and [reviewed](iteration_13/02_proposals/02_state_reconciliation_and_SC-031.md): state reconciled, WP0 baseline locked, and review diagnostic RD-1 run (zero solves). **SC-031 is `PROPOSED — NOT APPROVED FOR EXECUTION`**; nothing is running |
...
| Review / next decision | RD-1: all five prefixes that end above 2.5 mm RMS (ladder C, kite and peanut; M=2 kite and hook) collapse to a 1–2 mm tightest radius, the K=192 storage scale, within 2–5 clipped Gauss–Newton steps of stage 1 (the M=2 hook: stage 2). A 6-mm displacement cap does not prevent the collapse. Extra old-data work helps only the clean paths. **Next: the user's decision on "approve SC-031"** (Hanke regularizing LM with a curvature-change metric versus an L² control; stage-1 screen gate; ≤25,000 units, ≤3 h). SC-027 would be superseded. WP1/WP2 rollouts, WP4 and WP5 are deferred. Use SPD-008 as the SPD reference and exact cache reuse for new hybrid runs |
