...
class Ledger(p.Ledger):
    ...
    def reserve(self, maximum):
        ...
        if self.total >= self.cap and maximum+overhead > 0:
            raise TrialSolveCap('actual total solve cap reached')
        if (self.stage_quota is not None and
                self.total-self.stage_start+maximum+overhead > self.stage_quota and
                self.stage_quota-(self.total-self.stage_start) <= self.cap-self.total and
                self.total+overhead <= self.cap):
            raise StageQuota('next complete batch and endpoint reserve exceed planned stage quota')
        if self.total + maximum + overhead > self.cap:
...
def fit_stage(initial, data, nodes, solve, optimizer, floor, ledger, output):
    ...
    write(output/'optimizer.json',dict(config=asdict(optimizer), **context,
    ...
    try:
        ...
        minimum = m + 2*q*m + 3*m + 12
        write(output/'reservation.json',dict(reduced_directions=q, frequencies=m,
              complete_jacobian_bound=2*q*m, initial_objective=m,
