...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| KRESS | 9.754e-01 | 1.690e-03 | 3.519e+05x | 6.394e-02 | 8.781e-14 | 0.0241 | 0.0174 | 1.0382 | 38.31 s |

KRESS: `no_decreasing_neural_step` after 43 accepted updates / 334 forward evaluations.
...
- **PASS** `kress_training_loss_drop`: value `351858.2765171733`, required `>= 100`.
- **FAIL** `kress_boundary_error_within_representation`: value `{'final': 0.0010381669923228767, 'floor': 0.00023475232129879092}`, required `<= 2x the same network fitted to the exact target`.
- **PASS** `kress_boundary_error_improved`: value `{'final': 0.0010381669923228767, 'initial': 0.04325939264716375}`, required `<= 0.25x the initial contour error`.
- **FAIL** `kress_holdout_within_representation`: value `{'final': 0.0639440547749601, 'floor': 0.015242010519366297}`, required `<= 2x the same network fitted to the exact target`.
- **PASS** `kress_linear_system_residual`: value `7.0235443648012194e-15`, required `<= 1e-10`.
- **PASS** `kress_holdout_relative_l2`: value `0.0639440547749601`, required `<= 0.15`.
...
/home/drdeng/miniconda3/envs/EMNerf/bin/python \
  run_implicit_mlp_inverse.py --target circle --initial-model siren_circle --solvers kress --optimizer adjoint --max-iterations 60 --num-pairs 12 --output-dir results/inverse/implicit_mlp/circle-20260907T151516
```
