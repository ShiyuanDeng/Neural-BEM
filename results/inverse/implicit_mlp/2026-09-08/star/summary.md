...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| KRESS | 1.196e+00 | 4.824e-01 | 6.171e+00x | 8.627e-01 | 1.091e-07 | 7.2340 | 11.3154 | 37.3679 | 5353.74 s |

| Solver | amplitude error | rotation error radians |
|---|---|---|
| KRESS | 2.293e-01 | 1.691e-02 |

KRESS: `no_decreasing_neural_step` after 47 accepted updates / 420 forward evaluations.
...
- **FAIL** `kress_training_loss_drop`: value `6.171477848975452`, required `>= 100`.
- **FAIL** `kress_boundary_error_within_representation`: value `{'final': 0.03736793823807192, 'floor': 0.0011911248509951348}`, required `<= 2x the same network fitted to the exact target`.
- **FAIL** `kress_boundary_error_improved`: value `{'final': 0.03736793823807192, 'initial': 0.04274480692501427}`, required `<= 0.25x the initial contour error`.
- **FAIL** `kress_holdout_within_representation`: value `{'final': 0.8626503986605771, 'floor': 0.022813139555596988}`, required `<= 2x the same network fitted to the exact target`.
- **PASS** `kress_linear_system_residual`: value `1.7891787457328232e-14`, required `<= 1e-10`.
- **FAIL** `kress_holdout_relative_l2`: value `0.8626503986605771`, required `<= 0.15`.
- **PASS** `kress_warm_start_is_eikonal`: value `0.22758085575631748`, required `<= 0.25`.
...
/home/drdeng/miniconda3/envs/EMNerf/bin/python \
  /home/drdeng/Neural_SDF_BEM_AD/run_implicit_mlp_inverse.py --target star --initial-model siren_star --solvers kress --optimizer adjoint --max-iterations 60 --num-pairs 12 --train-ghz 0.5,1.5 --holdout-ghz 0.25,1.0,2.5 --num-nodes 194 --bandwidth 96 --grid-resolution 513 --projected-samples 256 --mlp-hidden-features 64 --mlp-hidden-layers 2 --mlp-pretrain-steps 6000 --mlp-pretrain-eikonal-weight 0 --learning-rate 0.001 --eikonal-weight 0.01 --loss-tolerance 1e-12 --conversion-tolerance-mm 0.2 --max-backtracks 14 --output-dir /home/drdeng/Neural_SDF_BEM_AD/results/inverse/implicit_mlp/2026-09-08/star --no-gate
```
