...
import numpy as np
from .execution import hankel1, solve_dense

from ordered_boundary import OrderedBoundary2D, PeriodicCurve2D
from ordered_boundary.validation_cache import current_validation_cache, validation_timed
...
def _required_pair_clearance(first, second, settings):
    return max(
        settings.minimum_absolute_clearance,
        settings.minimum_clearance_in_weights
        * max(float(np.max(first.arc_length_weights)), float(np.max(second.arc_length_weights))),
    )


@validation_timed('pair_exact')
def _validate_component_pair(
    ...
    required = _required_pair_clearance(first, second, settings)
    if intersects:
        raise MultiComponentTopologyError(
...
def _axis_separation_certificate(first, second, settings):
    """Sufficient sampled-polygon separation, or None for the exact fallback.

    All segments lie in their coordinate boxes. An axis gap lower-bounds every
    segment distance and excludes nesting. The conservative roundoff allowance
    also protects the reference predicate, not just real-arithmetic geometry:

    * Its orientation test tolerates 64*eps*S*L, with S the larger component
      diameter and L the longest edge. Require union diameter <= 3*S; the
      orientation's subtraction/product roundoff bound is <16*eps*L*D,
      hence below that tolerance. A false proper crossing cannot be created.
    * Expanded boxes exclude its tolerance-based endpoint/touching test.
    * A clipped segment projection is a convex combination up to rounding.
      4096*eps*max(coordinate magnitude, S, required) safely exceeds the few
      arithmetic errors in that projection, distance norm and ray crossing.

    Extremely small/large scales and degenerate edges use the exact path to
    avoid underflow/overflow assumptions. Bounds are rounded outwards; an
    ambiguous threshold always falls back. This is never a measured clearance.
    """
    ...
    if not np.isfinite(magnitude) or magnitude > 1e100:
        return None
    scale = max(float(np.linalg.norm(amax-amin)), float(np.linalg.norm(bmax-bmin)))
    union = float(np.linalg.norm(np.maximum(amax,bmax)-np.minimum(amin,bmin)))
    if not 1e-100 <= scale <= 1e100 or not union <= 3*scale:
        return None
    for points in (a,b):
        lengths = np.linalg.norm(np.roll(points,-1,axis=0)-points, axis=1)
        if not np.all(np.isfinite(lengths)) or np.any(lengths < 1e-100):
            return None
    required = _required_pair_clearance(first, second, settings)
    if not np.isfinite(required) or required > 1e100:
        return None
    eps = np.finfo(float).eps
    margin = np.nextafter(4096*eps*max(magnitude,scale,required), np.inf)
    length_tolerance = 64*eps*scale
    lower = np.maximum(np.nextafter(bmin-amax,-np.inf), np.nextafter(amin-bmax,-np.inf))
    expanded_gap = np.maximum(
        np.nextafter(bmin-length_tolerance,-np.inf)-np.nextafter(amax+length_tolerance,np.inf),
        np.nextafter(amin-length_tolerance,-np.inf)-np.nextafter(bmax+length_tolerance,np.inf))
    threshold = np.nextafter(required+margin, np.inf)
    valid = (lower > threshold) & (expanded_gap > margin)
    return float(np.max(lower[valid])) if np.any(valid) else None


def validate_multicomponent_admissibility(boundary, *, config=None):
    """Boolean-use validation; detailed reports still use the ordinary adapter.

    This function raises the same public errors, preserving exact fallback for
    every pair that cannot be certified. It never substitutes bounds into a
    ComponentPairReport or validates source/receiver acquisition geometry.
    """
    ...
    for index, component in enumerate(boundary.components):
        _component_adapter(component, context=f'boundary component at index {index}')
    cache = current_validation_cache()
    for i, first in enumerate(boundary.components):
        for second in boundary.components[i+1:]:
            if first.component_id == second.component_id:
                raise MultiComponentTopologyError('distinct components must have distinct component_id values.')
            certificate = (_axis_separation_certificate(first,second,settings)
                           if cache is not None and cache.mode == 'certified' else None)
            if certificate is not None:
                cache.counts['pair.certified'] += 1
                continue
            if cache is not None:
                cache.counts['pair.fallback'] += 1
            _validate_component_pair(first, second, settings)
...
def solve_multicomponent_kress_tmz_total_field_batch(
    ...
    *,
    ...
    solution = solve_dense(system.system_matrix, right_hand_side)
