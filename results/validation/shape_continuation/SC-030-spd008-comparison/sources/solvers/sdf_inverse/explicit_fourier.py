...
class CartesianFourierCurveState:
    ...
    def boundary_curve(self, config):
        ...
        settings = BoundaryValidationConfig(
            num_samples_per_component=config.validation_resolution,
            fourier_bandwidth=self.maximum_mode)
        try:
            report = memoized_validation('component_report', lambda: (
                array_key(self.cosine_coefficients), array_key(self.sine_coefficients),
                curve.component_id, curve.name, curve.period, curve.parameter_origin,
                tuple(asdict(curve.provenance).items()), tuple(asdict(settings).items()),
                array_key(config.bounds)),
                lambda: validate_periodic_parameterization(curve, settings, raise_on_error=False))
            if not report.valid:
