...
def spd_matching_policy(observations, optimizer, top025, update_modes=UPDATE_MODES):
...
def run_hybrid(inputs, directory, update_modes=UPDATE_MODES):
    top025 = inputs["top025"]
    ...
    policy = spd_matching_policy(observations, inputs["optimizer"], top025, update_modes)
...
def prepare(output, update_modes=UPDATE_MODES, experiment="SC-020"):
    ...
    manifest = dict(experiment=experiment, scene=SCENE, arms=list(ARMS), provenance=provenance(),
...
def run_one(output, arm):
    ...
    inputs = load()
    record = (run_spd(inputs, directory) if arm == "spd" else
              run_hybrid(inputs, directory, manifest["mapping"]["update_modes"]))
...
def main():
    ...
    parser.add_argument("--update-modes", type=int, default=UPDATE_MODES,
                        help="Hybrid update band M, fixed at preparation (SC-020: 16).")
    parser.add_argument("--experiment", default="SC-020")
    args = parser.parse_args()
    if args.prepare:
        prepare(args.output, args.update_modes, args.experiment)
    if args.one:
