"""Rebuild SC-032 tables and figures from saved evidence (no fitting, no forward solves).

Four-stage endpoints of R1/R2 (this bundle, resumed from SC-031 stage A) against R0 (SC-029).
Run from the repository root: PYTHONPATH=solvers:. python results/validation/shape_continuation/SC-032-regularizing-metric-prefix/report.py
...
def main():
    ...
    for arm in ARMS:
        for case in CASES:
            ...
            rows[(arm, case)] = dict(arm=arm, case=case, status=result["status"], reason=result.get("reason"),
                ...
                final_loss=result["stages"][-1]["final_loss"], final_stage=result["stages"][-1]["stage"],
    ...
    gate = json.loads(gate_path.read_text()) if gate_path.exists() else dict(released=None)
...
def table(rows, ratios, added, decision):
    lines = ["| Case | Arm | Status | RMS, mm | Hausdorff, mm | Stage-1 loss | Final loss (stage) | Units | Stage-1 end radius, mm | Min radius, mm | Refit / self-int. refusals |",
             "|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for case in CASES:
        for arm in ARMS:
            r = rows[(arm, case)]
            ...
            lines.append(f"| {case} | {arm} | {status} | {r['rms_mm']:.4g} | {r['hausdorff_mm']:.4g} | {r['stage_1_final_loss']:.3g} | {r['final_loss']:.3g} ({r['final_stage'][-1]}) | {r['work_units']} | "
...
def figures(rows):
    ...
    for ax, case in zip(axes.flat, CASES):
        ...
        ax.axhspan(5, 7, color="0.85", label="SC-031 gate band: collapse < 5 mm, avoided >= 7 mm")
        ax.set_title(case)
