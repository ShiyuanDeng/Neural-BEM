...
|---|---|
| [SC-041 outsider review](SC-041-outsider-review/README.md) | Kite's sharpest point (radius 0.082 mm) is a spurious flank staircase 5 mm from the true tip that carries the Hausdorff error; truth reaches loss 7e-29 on the same objective, so the data do not require it. Low-passing the state to K=32/64 removes it, and a K=32 release never forms it; a matched K=192 control from a clean state performs the same over 5 steps. Kite M=28 RMS 0.0537, Hausdorff 0.16 mm (SC-041 best 0.0730/0.342); star M=37/K=64 RMS 0.0181 mm (0.0476). Exploratory, noiseless (inverse crime); `audit.py` needs gitignored inputs. |
| [SC-041 atlas decisions](SC-041-atlas-decisions/README.md) | COMPLETE: corrected complete-update diagnostic and five matched continuations. Star M=19/22/25 RMS 0.142/0.110/0.0476 mm; kite M=19/22 RMS 0.1023/0.0730 mm, with a sharper point at M=22 and a numerical stop. Nominal RMS radius does not ensure finite validity. All five endpoint audits, 61 artifact checks and 24 targeted tests pass. 2,634 total work units; no controller/default promoted. |
| [SC-040 six-scene pipeline](SC-040-six-scene-pipeline/README.md) | COMPLETE. The current pipeline completed on circle and hook (state band, then release) and on star and peanut (release from SC-035 stage 4), with SC-035/SC-038 settings unchanged. All four complete their schedules at 512/1024 nodes, and every endpoint audit passes. Final RMS: circle 0.00024, star 0.142 (tips blunt: radius 8.99 vs 5.11 mm), peanut 0.0104, hook 0.0297 mm. 54 new atlas states. |
...
| [Six-scene pipeline videos](videos/README.md) | Rendering only, no solves: one trajectory per scene through the current pipeline (state band, then M 11/15/19 on all 19 frequencies), each with its atlas. Final RMS: circle 0.00024, star 0.142, C 0.0241, kite 0.110, peanut 0.0104, hook 0.0297 mm. Every atlas reproduces its run's saved loss. The earlier three-method comparison is in [videos/three_method_comparison](videos/three_method_comparison/README.md). |
...
| [SC-036 coordinate review](SC-036-coordinate-review/README.md) | Fixed-geometry diagnostics (15 units). Arclength-normal and radial 7-D spaces coincide on the circle and diverge off it (least principal cosine 0.40 on kite). Same-motion chain rule `J_alt = J_normal B` converges with resolution. The kite K=8 fit breaks RD-4's degree-only radius bound. |
...
| [SC-034 SPD legacy controls](SC-034-spd-legacy-controls/README.md) | COMPLETE: 18 SPD fits (ladder L, step controls S, both LS). H1 PASS (LS circle 1.6e-5, star 4.7e-5 mm). The ladder is the operative control: L reaches circle 7.6e-6, star 3.3e-5, peanut 1.2e-5 mm, kite 1.25 mm (certificate stop). The step controls alone at K17 are worse than SPD-008. C and hook are not star-shaped, so SPD cannot represent them. |
...
| [SC-028 preflight](SC-028-atlas-strategies/README.md) | Full P=48 refinement fails on C/kite/peanut; recovery dispatch was gated off. The separately recorded [active-band diagnostic](SC-028-active-band-diagnostic/README.md) passes every M<=19 column at the unchanged tolerance and motivates SC-029's narrower qualification. |
| [SC-026 atlas dataset](SC-026-atlas-dataset/README.md) | 1,286 unique curves from 49 trajectories and six cases × 19 frequencies. The [independent audit](SC-026-independent-audit/README.md) confirms hashes/algebra and qualifies the first analysis: sensitive columns are not jointly identifiable modes, the noise scale is assumed, and normal-ray coverage is often incomplete. All 24 distinct pathological transitions occur in stage 1. |
| [SC-025 band policies](SC-025-band-policies/README.md) | Ladder, fixed M=32, progress controller and the frozen A2 atlas rule, on 3 development and 3 held-out cases (backend V2). A2 wins large on the C (0.37 against 3.20 mm) and the held-out peanut (0.31 against 2.94 mm), and loses on the held-out kite and hook. **Not established as better than the ladder.** Fixed M=32 fails everywhere; the progress controller never triggers. |
| [SC-024 backend ablations and probes](SC-024-backend-ablations/README.md) | 18 variant runs plus an iteration-cap diagnostic. The relaxed refit gate helps the C most (Hausdorff 19.4 → 11.8 mm); fixed M=32 fails under every variant. 110 executed probe steps: realized/predicted 1.04, 95% accepted; wide bands overfit near convergence. |
| [SC-023 conditional candidates](SC-023-conditional-candidates/README.md) | Q0: the atlas numerics pass refinement (12/12) and directional checks (24/24); the refit gate froze roughened curves. 335,016 offline candidates: no declared rule beats the ladder; the raw model decrease ranks bands backwards. Amendment A1 is not qualified; A2 (parsimonious) qualifies on development data only. |
