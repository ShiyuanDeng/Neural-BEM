...
|---|---|
| [SC-022 atlas survey](SC-022-atlas-survey/README.md) | Full atlas at every accepted state of fixed-schedule trajectories: wrong circle, star and non-star C; 19 frequencies × 48 harmonics; 2,679 cells reproducing the backend's steps to ≤1.5e-13. Fixed M=32 stalls from far starts; Borges' ladder recovers the circle (0.005 mm), gets the star to 1.43 mm and fails on the C (19.4 mm). Descriptive. |
...
| [SC-020 clean hybrid vs SPD](SC-020-spd-matched-hybrid/README.md) | **PASS.** Borges update, SPD-style LM and SPD's cumulative schedule on SPD's merge case. The fresh SPD rerun is hash-identical to TOP-025. The hybrid passes every SPD gate with every endpoint qualified. Final 0.076 against 0.048 mm, 580 against 256 units, 203 against 143 s; the cause is the M=16 update band's 0.050 mm out-of-band floor. See [iteration 07](../../../docs/iterations/shape_frequency_continuation/iteration_07/01_results.md). |
