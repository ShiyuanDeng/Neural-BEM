...
class Ledger:
    def __init__(self, cap=5000, seconds=1200., clock=time.perf_counter):
        ...
        self.compiled_attempted, self.compiled_completed, self.compiled_failed = Counter(), Counter(), Counter()
        self.compiled_frequency_attempted, self.compiled_frequency_completed, self.compiled_frequency_failed = Counter(), Counter(), Counter()
        self.calls = Counter()
    ...
    def budget_total(self):
        return (self.total + sum(self.derivative_attempted.values()) + sum(self.reciprocal_attempted.values())
                + sum(self.compiled_attempted.values()))

    def analytic_event(self, event, kind, omega, error):
        if kind not in ('base', 'direction', 'reciprocal', 'compiled'):
            raise ValueError('Unknown analytic work kind: '+kind)
        if event == 'attempt':
            self.reserve(1)
        category = self.category
        prefix = {'base':'', 'direction':'derivative_', 'reciprocal':'reciprocal_', 'compiled':'compiled_'}[kind]
        ...
        if kind == 'compiled':
            key = f'{getattr(self, "stage", None)}:{category}:{float(omega)/(2*np.pi):.0f}'
            getattr(self, 'compiled_frequency_'+('attempted' if event == 'attempt' else event))[key] += 1
    ...
    def snapshot(self):
        return dict(attempted=dict(self.attempted), completed=dict(self.completed),
                    ...
                    compiled_batches_attempted=dict(self.compiled_attempted),
                    compiled_batches_completed=dict(self.compiled_completed),
                    compiled_batches_failed=dict(self.compiled_failed),
                    compiled_per_frequency_attempted=dict(self.compiled_frequency_attempted),
                    compiled_per_frequency_completed=dict(self.compiled_frequency_completed),
                    compiled_per_frequency_failed=dict(self.compiled_frequency_failed),
                    budget_work_units=self.budget_total,
                    budget_unit='one full frequency system, operator direction/tangent, reciprocal RHS batch, or compiled frequency batch')
