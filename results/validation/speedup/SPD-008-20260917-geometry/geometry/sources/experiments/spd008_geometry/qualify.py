...
def remaining_budget(bundle, phase, seconds, units=None):
    """Preserved diagnostic attempts count against the original total ceiling."""
    prior = [c.read(path) for path in bundle.glob(phase+'_attempt_*/qualification.json')]
    seconds -= sum(row['seconds'] for row in prior)
    if units is not None:
        units -= sum(row['work']['budget_work_units'] for row in prior)
    if seconds <= 0 or (units is not None and units <= 0):
        raise RuntimeError('Original '+phase+' diagnostic budget exhausted')
...
def geometry(bundle):
    ...
    seconds, _ = remaining_budget(bundle,'geometry',300)
    solve = c.p.driver.baseline.iteration01_solve_config()
    try:
        with c.wall_limit(seconds):
    ...
    result = dict(status=status,error=error,rows=rows,seconds=time.perf_counter()-started,
                  source_sha256=frozen, physical_solves=0, wall_ceiling_seconds=seconds,
                  original_total_wall_ceiling_seconds=300)
...
def updates(bundle):
    ...
    frozen = c.freeze(output,inputs)
    seconds, units = remaining_budget(bundle,'updates',1800,2000)
    ledger = c.m.Ledger(cap=units,seconds=seconds)
    rows, status, error = [], 'PASS', None
    started = time.perf_counter()
    profile_row = None
    try:
        with c.wall_limit(seconds), inverse_runtime('compiled'), execution(kernels='real_bessel'), ledger.instrument():
            for rep in range(2):
                for scene in c.SCENES[2:]:
    ...
    result = dict(status=status,error=error,rows=rows,profile=profile_row,gates=gates,
                  ...
                  work=ledger.snapshot(),source_sha256=frozen,limits=dict(seconds=seconds,work_units=units),
                  original_total_limits=dict(seconds=1800,work_units=2000))
    c.write(output/'qualification.json',result)
    if status!='PASS':
        raise RuntimeError(error)
    return result
