...
def prepare(bundle):
    ...
    c.freeze(out,[bundle/'tests.json',bundle/'tests.log',bundle/'geometry/qualification.json',
                  bundle/'updates/qualification.json'])
    ...
    manifest['input_sha256'][str((out/'approved_plan.md').relative_to(c.ROOT))] = c.sha(out/'approved_plan.md')
    for arm in c.ARMS:
        for rep in range(2):
            for path in (out/f'{arm}_{rep}').rglob('*.json'):
                manifest['input_sha256'][str(path.relative_to(c.ROOT))] = c.sha(path)
    spent = sum(c.read(path)['seconds'] for path in bundle.glob('campaign_attempt_*/execution_status.json'))
    if spent >= 10800:
        raise TimeoutError('Original campaign budget exhausted')
    manifest.update(campaign_seconds=10800-spent,original_total_campaign_seconds=10800,
                    previous_attempt_seconds=spent,worker_seconds=2400,maximum_concurrent_workers=1,
...
def campaign(bundle):
    out = prepare(bundle)
    campaign_seconds = c.read(out/'manifest.json')['campaign_seconds']
    jobs = []
    for rep in range(2):
