...
import numpy as np
import pytest
from experiments.spd004_pipeline_readiness.gate import training_readiness
from experiments.spd004_pipeline_readiness import run as r
...
def test_ready_route_still_checks_independent_quality(tmp_path,monkeypatch,stubbed,independent_pass):
    monkeypatch.setattr(r,'ORIGINAL_CONTINUATION',lambda *a:pytest.fail('ready state must skip fitting'))
    def score(*a):
...
def test_unready_route_preserves_original_fallback_and_charges_screen(tmp_path,monkeypatch,stubbed):
    ...
    monkeypatch.setattr(r,'ORIGINAL_CONTINUATION',original)
