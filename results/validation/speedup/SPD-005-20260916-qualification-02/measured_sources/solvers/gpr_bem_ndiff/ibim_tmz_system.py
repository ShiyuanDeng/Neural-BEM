"""Muller system assembly, solve, and receiver evaluation for ``gpr_bem_ndiff``.

Unchanged from ``gpr_bem_kdiff`` except in wording; the operator blocks it
consumes are built by ``ibim_tmz_forward.build_kdiff_operator_blocks``, whose
implementation this fork replaces.
