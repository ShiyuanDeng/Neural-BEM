...
from sdf_inverse.runtime import add_runtime_argument, configure_runtime_argument, runtime_metadata, inverse_execution, current_runtime, ENVIRONMENT_VARIABLE
...
def prepare(bundle, validation):
    ...
    from experiments.top025.readiness import SCREEN_SOLVE_CAP, SCREEN_SECONDS
    readiness = current_runtime().continuation_readiness
    screen_cap, screen_seconds = (SCREEN_SOLVE_CAP, SCREEN_SECONDS) if readiness else (0, 0)
    write(bundle/'contract.json',dict(experiment='TOP-025', approval='user requested latest-code all-case evaluation and videos',
        ...
        continuation_solve_cap=8012+screen_cap,continuation_seconds=7200+screen_seconds,
        continuation_fit_solve_cap=8012,continuation_fit_seconds=7200,
        readiness=readiness,readiness_solve_cap=screen_cap,readiness_seconds=screen_seconds,
        oracle_solve_cap=72,oracle_seconds=300,
        campaign_solve_cap=TOTAL_CAP+12*screen_cap,campaign_seconds=WALL_SECONDS,maximum_workers=WORKERS,
        ...
        inverse_runtime=runtime_metadata(),budget_unit='full system, operator direction, reciprocal RHS batch, or compiled frequency batch',
...
def verify(bundle):
    require(read(bundle/'contract.json').get('inverse_runtime',runtime_metadata())==runtime_metadata(),
            'inverse runtime differs from the prepared campaign')
...
def run_continuation(folder,state,optimizer,observed,evaluation,scene,spec,control,solve):
    """Default full inverse: training readiness, then the unchanged schedule if needed."""
    if current_runtime().continuation_readiness:
        from experiments.top025.readiness import readiness_continuation
        return readiness_continuation(folder,state,optimizer,observed,evaluation,scene,spec,control,solve)
    return run_scheduled_continuation(folder,state,optimizer,observed,evaluation,scene,spec,control,solve)


@inverse_execution
def run_scheduled_continuation(folder,state,optimizer,observed,evaluation,scene,spec,control,solve):
...
def run_job(bundle,scene_id,deadline):
    ...
    # ThreadPoolExecutor does not inherit the caller's ContextVar override.
    profile=read(bundle/'contract.json')['inverse_runtime']['profile']
    env={**os.environ,ENVIRONMENT_VARIABLE:profile,**{k:'1' for k in ('OMP_NUM_THREADS','OPENBLAS_NUM_THREADS','MKL_NUM_THREADS','NUMEXPR_NUM_THREADS')}}
