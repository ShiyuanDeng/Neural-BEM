...
from sdf_inverse.runtime import add_runtime_argument, configure_runtime_argument, runtime_metadata, inverse_execution
...
def run_case(name, output, args):
    ...
    config = replace(config,
        ...
        refined_feasibility_guard=getattr(args, 'refined_feasibility_guard', False),
    ...
    write_json(path / 'manifest.json', dict(case=name, demonstration=args.demonstration,
        ...
        chart=args.chart, controller=asdict(config), oracle=oracle, inverse_runtime=runtime_metadata(),
...
def main():
    ...
    parser.add_argument('--refined-feasibility-guard', action='store_true',
                        help='Require every optimizer step to be admissible at the refined discretization too.')
