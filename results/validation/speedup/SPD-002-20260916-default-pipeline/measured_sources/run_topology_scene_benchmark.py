...
def verify_source(output):
    if read(output / 'manifest.json').get('inverse_runtime', runtime_metadata()) != runtime_metadata():
        raise ValueError('Inverse runtime differs from the prepared benchmark.')
...
def run_one(output, scene_id, arm):
    ...
    driver.write_json(path / 'manifest.json', dict(scene=scene_id, arm=arm, controller=asdict(config), inverse_runtime=runtime_metadata(),
        source=read(output / 'manifest.json')['source'],
