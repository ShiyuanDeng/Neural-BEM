...
class TrialLedger(p.Ledger):
    def __init__(self,cap,seconds):
        ...
        self.stage=0;self.endpoint=False;self.endpoint_reserve=12;self.stage_work_start=0

    def reserve(self,maximum):
        overhead=0 if self.endpoint else self.endpoint_reserve
        super().reserve(maximum+overhead)
        remaining=self.stage_cap-(self.budget_total-self.stage_work_start)
    ...
    def begin_stage(self,number,cap):
        self.stage=number;self.stage_start=self.total;self.stage_cap=cap;self.stage_work_start=self.budget_total
