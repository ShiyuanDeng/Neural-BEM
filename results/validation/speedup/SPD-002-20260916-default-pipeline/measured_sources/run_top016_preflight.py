...
class Ledger:
    ...
    def budget_total(self):
        return self.total + sum(self.derivative_attempted.values())

    def analytic_event(self, event, kind, omega, error):
        if event == 'attempt':
            self.reserve(1)
        category = self.category
        counters = (dict(attempt=self.attempted, completed=self.completed, failed=self.failed)
                    if kind == 'base' else
                    dict(attempt=self.derivative_attempted, completed=self.derivative_completed, failed=self.derivative_failed))
