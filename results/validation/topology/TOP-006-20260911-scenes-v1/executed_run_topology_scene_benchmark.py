...
def plot_geometry(ax, scene, state, initial=None):
    ...
    ax.set(xlim=(.28, .72), ylim=(.28, .74), aspect='equal')
    ax.grid(alpha=.12)
...
def render_results(output, spec, rows):
    ...
    for group in ('original', 'new'):
        ...
        for i, scene in enumerate(scenes):
            for j, arm in enumerate(ARMS):
                ...
                state = None if row is None else driver.deserialize_state(row['final_state'])
                plot_geometry(ax, scene, state, initial_state(scene))
                detail = 'INCOMPLETE' if row is None else (
    ...
    for ax, arm in zip(axes[1:], ARMS):
        ...
        ax.set_title(f"{'Default A' if arm=='A' else 'Selective F'} — "
                     + ('incomplete' if row is None else ('PASS' if row['passed'] else 'FAIL')))
    fig.savefig(output / 'far_ellipse_star.svg')
    fig.savefig(output / 'far_ellipse_star.png', dpi=160)
    plt.close(fig)
    for arm in ARMS:
        path = output / 'runs' / arm / scene['id']
        if not (path / 'trajectory.json').exists():
            continue
        frames = tuple(TopologyFrame(driver.deserialize_state(r['state']), r['loss'], r['label'], r['cycle'])
                       for r in read(path / 'trajectory.json'))
        if frames and not (path / 'inversion.mp4').exists():
            driver.render_case(path, f'distant circle to ellipse/star / {arm}', truth_curves(scene),
                               frames, read(path / 'events.json'))
