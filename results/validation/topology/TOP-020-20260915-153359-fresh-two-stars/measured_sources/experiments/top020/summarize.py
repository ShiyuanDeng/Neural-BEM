...
def verify(output):
    ...
    for stage in schedule.get('stages', []):
        ...
        if 'score' in stage:
            ...
            assert stage['work_after_endpoint']['stage_attempted'] <= contract['stage_plan'][number-1][1]
    ...
    for work, cap, seconds in zip(works, (4000, 8012), (600, 7200)):
        assert work['total_attempted'] == sum(work['attempted'].values()) == sum(work['frequency_attempted'].values()) <= cap
...
def figure(output, endpoints):
    ...
    fig.suptitle('TOP-020 · Fresh automatic two-star recovery')
    fig.tight_layout()
