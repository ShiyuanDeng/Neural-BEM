...
class Ledger:
    ...
    def reserve(self, maximum):
        ...
        if self.total + maximum > self.cap:
            raise Budget(f'batch of {maximum} cannot fit remaining {self.cap-self.total} frequency solves')
    ...
    def snapshot(self):
        return dict(attempted=dict(self.attempted), completed=dict(self.completed),
                    ...
                    within_solve_cap=self.total <= self.cap)
