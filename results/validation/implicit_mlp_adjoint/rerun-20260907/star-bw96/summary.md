...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| KRESS | 1.228e+00 | 5.966e-01 | 4.430e+00x | 1.078e+00 | 1.102e-07 | 15.5084 | 4.8332 | 36.3864 | 2995.14 s |

| Solver | amplitude error | rotation error radians |
|---|---|---|
| KRESS | 1.435e-01 | 5.253e-01 |

KRESS: `no_decreasing_neural_step` after 32 accepted updates / 233 forward evaluations.
...
- **FAIL** `kress_training_loss_drop`: value `4.429515902610703`, required `>= 100`.
- **FAIL** `kress_boundary_error_within_representation`: value `{'final': 0.036386374933877585, 'floor': 0.0011911248509951348}`, required `<= 2x the same network fitted to the exact target`.
- **FAIL** `kress_boundary_error_improved`: value `{'final': 0.036386374933877585, 'initial': 0.04274480692501427}`, required `<= 0.25x the initial contour error`.
- **FAIL** `kress_holdout_within_representation`: value `{'final': 1.078094966718769, 'floor': 0.02500479609256957}`, required `<= 2x the same network fitted to the exact target`.
- **PASS** `kress_linear_system_residual`: value `2.7727558258563106e-14`, required `<= 1e-10`.
- **FAIL** `kress_holdout_relative_l2`: value `1.078094966718769`, required `<= 0.15`.
...
/home/drdeng/miniconda3/envs/EMNerf/bin/python \
  /tmp/claude-1001/-home-drdeng-Neural-SDF-BEM-AD/caa6f2cb-1251-4a2c-a99c-48e759f67062/scratchpad/run_star_bw96.py --target star --max-iterations 60 --num-pairs 8 --num-nodes 194 --no-gate --output-dir results/validation/implicit_mlp_adjoint/rerun-20260907/star-bw96
```
