...
[failure audit](../failure-audit-20260907/README.md). These repair demonstrated
...
The [short star inverse](star-smoke/summary.md) uses the final star defaults,
...
[subsequent reruns](../rerun-20260907/README.md) do that, and show the
...
[validation.json](validation.json) records commands, scope and final source
hashes. `git diff --check` passes.

The evidence in [metrics.json](metrics.json) includes the circle restart and
saved conversion checks. The same file's `star_target_fit` entry and
[star_with_offsets.json](star_with_offsets.json) are **rejected prototypes**:
a broader resolved-distance redesign produced 3 or 23 zero contours and was
excluded from production. [ellipse_initial_fit.json](ellipse_initial_fit.json)
...
See the [pipeline guide](../../../../docs/pipelines/implicit_mlp.md) for current
